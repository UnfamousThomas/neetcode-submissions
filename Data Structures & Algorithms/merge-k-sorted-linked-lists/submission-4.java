...
class Solution {
    public ListNode mergeKLists(ListNode[] lists) {
        if(lists.length == 0) {
            return null; //Return empty if no lists to sort
        }

        for(int i = 1; i < lists.length; i++) { //Loop through the lists
            //and merge them incrementally, eventually lists[n] has the merged head
